...
void sorts::swap(vector<int>& vec, int index1, int index2) {
    int temp = vec[index1];
    vec[index1] = vec[index2];
    vec[index2] = temp;
}

void sorts::BubbleSort(vector<int>& vec, int start, int end) {
    int length = vec.size();
...
void sorts::InsertionSort(vector<int>& vec, int start, int end) {
    int length = vec.size();
...
void sorts::MergeSort(vector<int>& vec, int start, int end) {
    if (start < end) {
...
void sorts::merge(vector<int>& vec, int left, int mid, int right, vector<int>& temp) {
    int i = left;
    int j = mid;
    int k = left;
    while ((i < mid) && (j < right)) {
...
void sorts::IterativeMergeSort(vector<int>& vec, int start, int end) {
...
int sorts::median(int val1, int val2, int val3) {
...
int sorts::partition(vector<int>& vec, int left, int right) {
    int pivot = vec[right];
    int i = left - 1;
...
int sorts::medianPivot(vector<int>& vec, int left, int right) {
    int mid = right / 2;
...
void sorts::quickSortHelper(vector<int>& vec, int left, int right) {
    if (left < right) {
...
void sorts::QuickSort(vector<int>& vec, int left, int right) {
    if (left >= right) {
        return;
    }
    else if (left < right) {
...
void sorts::ShellSort(vector<int>& vec, int start, int end) {
    int gap = vec.size() / 2;
    while (gap >= 1) {
